...
export function SuggestionBar() {
  ...
  return (
    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2
                    bg-white border border-gray-200 text-sm text-gray-700 px-4 py-2 rounded-lg shadow-md
                    max-w-lg">
      <span className="flex-1">{current.message}</span>
      {current.action && (
        <button
          onClick={() => handleAction(current.action!.nodeType)}
          className="px-2.5 py-1 text-xs font-medium bg-blue-100 hover:bg-blue-200 text-blue-700 rounded transition-colors whitespace-nowrap"
        >
